...
import { Search, MapPin, Phone, Star, CheckCircle, Calendar, ShieldCheck, XCircle, Clock, Check } from 'lucide-react';
...
export const ClientView: React.FC<ClientViewProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'DIRECTORY' | 'MY_APPOINTMENTS'>('DIRECTORY');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        <div className="flex gap-2">
          ...
          >
            {currentLang === 'FR' ? '🔎 Trouver un Professionnel' : '🔎 دليل المهنيين والأطباء'}
          </button>
          <button
            ...
            <div className="flex flex-col md:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-hidden focus:ring-2 focus:ring-teal-500 focus:bg-white transition"
                />
              </div>
              ...
              <div className="shrink-0 flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-slate-50 border border-slate-200 text-slate-700 text-sm font-medium rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-teal-500 focus:outline-hidden"
                >
                  {citiesList.map((c) => (
          ...
          {appointments.length === 0 ? (
            ...
            <div className="space-y-3">
              {appointments.map((app) => (
                <div
                  key={app.id}
                  className="p-4 rounded-xl border border-slate-200 hover:border-slate-300 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50"
                >
